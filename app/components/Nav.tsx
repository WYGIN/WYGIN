        ...
        <div className='p-0 block'>{/*search bar layout*/}
            ...
            <ul className='mb-2.5 box-border block list-disc'>
              {
                ...
                data.body.nav.linkGroup.forEach((link) => {
                  <li className='bg-[#4b576d] text-white relative block w-full list-none box-border'>
                    <a className='text-white no-underline relative block h-[50px] p-4 font-semibold text-center bg-transparent box-border'>`${link.link} &nbsp;`</a>
                          ...
                          <li className='md:p-0 md:mx-[5px] md:mt-0 md:mb-auto relative block border-b-2 box-border list-style-[circle] text-white'>
                            <a href={link.url} className='text-[#267438] no-underline flex bg-transparent list-style-[circle] '>
                              ...
                              <div className='m-0 text-[14.4px] xl:inline xl:max-w-[70%] mt-[5px] pl-[10px] relative text-white no-underline text-left grow shrink basis-auto box-border list-style-[circle]'>
                                <h3 className='text-xl font-semibold no-underline mt-[20px] mb-[10px] relative box-border block list-style-[circle]'>{link.label}</h3>
                                <p className='mb-0 mr-[10px] box-border block text-left text-style-[circle]'>{link.description}</p>
                              </div>
                            </a>
                          </li>
                        })}
          ...
          <div title='row' className='box-border block mx-0 md:px-[5%] text-xl'>
            <div title='col-md-6' className='box-border block text-white'>
              {
                data.body.nav.linkSection.forEach((link) => {
                  <section className='block box-border'>
                    <svg className='w-[35px] h-[35px] inline-block align-middle box-border'></svg>
                    <h4 className='pl-[5px] my-0 pb-0 font-bold inline-block align-middle text-[22px] no-underline box-border '>{link.label}</h4>
                    ...
                    <ul className='list-outside pl-0 mb-[40px] mt-0 box-border block'>
                      {link.group.forEach((link) => {
                        <li className='sm:block sm:w-[100%] pb-[12px] align-top pr-[20px] box-border list-outside '>
                          <a className='list-outside' href={link.link.url}>{link.label}</a>
                        </li>
                      })}
                    </ul>
                  </section>
